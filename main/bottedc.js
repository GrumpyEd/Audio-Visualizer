...
function shufflePlaylist() { // Shuffles playlist on click on canvas
...
const playlist = [ // List of .mp3 files that will start playing on first click
    "../pls/cry.mp3",
    "../pls/beedas.mp3",
    "../pls/0mel.mp3",
    "../pls/1.6.mp3",
...
canvas.addEventListener('click', function() { // On-Click on canvas function for starting the media
    if (isPlaying) {
        playNextTrack();
    } else {
        toggleAudio();
...
function drawLine(points) { // In cahoots with the math in the beginning of the file. 
    let origin = points[0];
...
function update(dt) {
    ...
    for (let i = 0; i < pointsUp.length; i++) { // Math
